...
def dataflow(beam_args: List[str] = None):
    options = PipelineOptions(beam_args, runner="DirectRunner", streaming=False)

    with beam.Pipeline(options=options) as pipe:
        input_data = pipe | "Read from GCS Bucket" >> beam.io.ReadFromText("gs://your-bucket-name/test-data.json")

        input_dict = input_data | "Convert to Dict" >> beam.Map(lambda elm: json.loads(elm))

        input_dict | "Print Input data" >> beam.Map(lambda elm: print(json.dumps(elm)))

        output_dict = input_dict | "Transform" >> beam.ParDo(transform_elements())

        output_dict | "Print Output data" >> beam.Map(lambda elm: print(json.dumps(elm)))
